...
package com.licon.syncadaptertest.syncadapter;
...
public class TvShowsSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    public void onPerformSync(Account account, Bundle extras, String authority,
        ...
        Log.d("licon-sync", TAG + "> onPerformSync for account[" + account.name + "]. Extras: " + sb.toString());
        ...
        try {
            ...
            if (showsToRemote.size() == 0) {
                Log.d("licon-sync", TAG + "> No local changes to update server");
            } else {
                Log.d("v", TAG + "> Updating remote server with local changes");

                // Updating remote tv shows
                for (TvShow remoteTvShow : showsToRemote) {
                    Log.d("licon-sync", TAG + "> Local -> Remote [" + remoteTvShow.name + "]");
                    parseComService.putShow(authToken, userObjectId, remoteTvShow);
                }
            }

            if (showsToLocal.size() == 0) {
                Log.d("licon-sync", TAG + "> No server changes to update local database");
            } else {
                Log.d("licon-sync", TAG + "> Updating local database with remote changes");
